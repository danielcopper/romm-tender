/**
 * Every QAM component's source, as TEXT — what a drift lock searches.
 *
 ...
 *
 * So the set is swept rather than listed: every `.tsx` under `src/components`
 * that is not itself a test. A component added tomorrow is searched because it
...
export interface ComponentSource {
  /** Relative to `src/`, POSIX-separated — the name a test case is reported under. */
  path: string;
...
export function componentSources(): ComponentSource[] {
  const files = globSync("components/**/*.tsx", { cwd: SRC_DIR })
    .filter((relative) => !relative.endsWith(".test.tsx"))
    .map((relative) => relative.split(/[\\/]/).join("/"))
    .sort();
  if (files.length === 0) {
    throw new Error(`No component sources found under ${SRC_DIR}components — a drift lock over nothing passes always.`);
  }
