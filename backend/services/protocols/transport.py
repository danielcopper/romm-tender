...
class LatestReleaseFn(Protocol):
    """Read this program's latest published release, or answer that none could be read.

    Implementations never raise and never report a failure: an unreachable
    server, an unreadable answer and a payload naming no Tender release all
    answer ``None``, because the caller's only output is a card it may quietly
    not show.
    """
