...
short label the reporter appends to the collection's name
(``RomM: [<name> (Franchise)] (host)``) — or onto no label, for a standard
collection, which keeps its bare name. No I/O, no state.
...
def collection_label(kind: str, virtual_type: str | None) -> str | None:
    ...
    missing or unrecognised (an older backend, a virtual type the plugin does
    not sync). An unknown ``kind`` degrades to the capitalised kind string
    (never expected for the ``standard`` / ``smart`` / ``virtual`` set), and an
    empty kind to ``"Collection"``. Every label is free of ``]`` so the
    reconcile name-parse stays intact.
    """
    if kind == "standard":
        return None
    if kind == "virtual":
