...
def _parse_version(version_str: str | None) -> tuple[tuple[int, ...], bool] | None:
    ...
    pre-release suffix, which both RomM's server versions and this program's
    own releases are spelled in.
    """
...
def is_newer_version(candidate: str | None, current: str | None) -> bool:
    ...
    Both take the shapes :func:`meets_min_version` accepts, and a pre-release
    ranks below its own release, so ``0.33.0-beta`` is not newer than
    ``0.33.0`` while ``0.34.0-beta`` is. Equality is never newer.

    Returns ``False`` whenever either side cannot be parsed: the caller offers an
    update on a True, so an unreadable version says nothing rather than
    announcing one.
    """
