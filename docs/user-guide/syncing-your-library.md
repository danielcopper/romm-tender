...
The **Collections** tab of the Library page lists the **kinds** of collection on the left, and the selected kind's
collections as a table on the right. The kinds are:

- **Favorites** — your own RomM favorites collection. Its row carries its sync switch and the number of games in it.
- **Collections** — the collections you (or, on a shared server, other users) picked by hand in RomM.
- **Smart collections** — saved searches, whose games are whatever the search matches when the sync runs.
- Under **Autogenerated**: **Franchises** and **IGDB collections** — groupings RomM builds from IGDB metadata. They have
  no owner.

Each kind's row says how many of its collections are on, for example `4 of 12 on`. The page opens on **Collections**.

**Turning a collection on adds all of its games to Steam — including games on platforms you have not synced.** A
collection is not limited to the platforms you switched on under Platforms: every game in it is added, and the games are
grouped in a Steam collection named after it, `RomM: [<name>] (<device>)`. Each kind's pane says this in its first
sentence, together with the name the Steam collection gets when **Distinguish collection types in Steam names** is on
(see [Collections that share a name](#collections-that-share-a-name)).

The table has these columns:

- **Collection** — the collection's name.
- **Owner** — _you_, or the RomM user name of the user it belongs to (Collections and Smart collections only).
...
type in the search to bring them into view.
...
search leaves, straight away. On **Franchises** or **IGDB collections** they only ever switch that one type. They leave
the collection behind the **Favorites** row alone, and while **Other users' collections** is off they never switch
someone else's.

If a switch does not take — for example while a RetroDECK path migration is pending — it flips back and a line says why:
in the pane for a table switch or Enable all / Disable all, and under **Other users' collections** for that switch or
the Favorites switch. The line goes away with the next change that works there.
...
**Other users' collections** switch at the top of the list — _Their public ones, shown and synced_ — decides whether
they count:
...
sign-ins pick it up on the next connection check). Until then every collection counts as yours, so turning it off hides
nothing.
