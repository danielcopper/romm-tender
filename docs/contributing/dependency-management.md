...
| `python`       | `mise.toml` + workflow `setup-python` + `pyproject.toml` + `sonar-project.properties` + `install.sh` | Tender runs on the system interpreter, and SteamOS ships Python 3.13: 3.13.1 in SteamOS 3.7 and 3.13.5 in 3.8, the current stable (Valve's package mirror, `core-3.7` / `core-3.8`) |
