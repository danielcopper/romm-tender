...
description of the Steam Library setting that turns that mode on, and the user guide: a standard collection, favorites
included, carries no suffix, and every other kind a short form of its row's name — `(Smart)`, `(Franchise)`,
`(IGDB Collection)` — with `(Autogenerated)`, its group heading's word, as the fallback for a virtual collection of no
known type. The rule, why it is shaped so, and the places the labels are spelled are
[steam-non-steam-shortcuts.md](steam-non-steam-shortcuts.md) § Collection naming mode. The keys on the wire and in
...
2. **Library** ([#1815](https://github.com/danielcopper/romm-tender/issues/1815)) — Platforms and Collections; System
   ...
   collections fix. Second on purpose: the emu-atlas work under #1735 renders its BIOS and core changes into the new
   Platforms detail instead of the retired System page. **Landed**: Platforms (#1836), then Collections in three code
   PRs — the backend's in-Steam count and owner name, the page as list and detail over the kinds, and the Steam-name
   suffixes that follow the kinds' names ([#1833](https://github.com/danielcopper/romm-tender/issues/1833)).
