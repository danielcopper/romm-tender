...
description of the Steam Library setting that turns that mode on, and the user guide. A standard collection carries no
suffix, a favorites collection being a standard one; every other kind gets a short singular of its row's name — a smart
collection `(Smart)`, a franchise `(Franchise)` and an IGDB collection `(IGDB Collection)`. The label kernel's fallback
for a virtual collection of no known type is `(Autogenerated)`, its group heading's word, and it is never written by the
sync, which fetches only the two known virtual types. `(IGDB Collection)` keeps its capital because Steam tells
collections apart without regard to case, and Tender finds an existing one the same way, so a change of case alone would
not rename a collection that already exists. [steam-non-steam-shortcuts.md](steam-non-steam-shortcuts.md) § Collection
naming mode states the rule and lists the places the labels are spelled. Because a standard collection carries no
suffix, a standard collection named "Foo (Smart)" and a smart one named "Foo" have the same Steam name and merge into
one; that is accepted, since it takes a name spelling out another kind's suffix. A Steam collection named for a standard
collection with a `(Standard)` suffix is absent from the sync's set, so the next sync that is not cancelled creates the
bare-named collection and removes the `(Standard)` one, as it removes any collection absent from its set — a new Steam
collection in the old one's place, not the old one renamed. The keys on the wire and in `settings.json` stay `standard`
/ `smart` / `virtual`.
...
2. **Library** ([#1815](https://github.com/danielcopper/romm-tender/issues/1815)) — Platforms and Collections; System
   ...
   landed, and so has Collections** as list and detail over the kinds, with the Steam-name suffixes that follow the
   kinds' names ([#1833](https://github.com/danielcopper/romm-tender/issues/1833)).
