...
- **BIOS files** — the summary (required, or files, the console's own demand where it has one, and the three shapes that
  make no readiness claim — `system_image: "absent"` reads "Needs one of these" and outranks the counts and the decline
  alike, tested before either, because the console asks for one of the images and no count can state that; `"unsettled"`
  joins `required_withheld` on the keeping side of `nothingEstablished`, so its downloads stay), then a table: File, On
