...
with the owner's name; the member ids do not cross the wire. `get_collections` receives both today and forwards neither,
and the page still costs the four RomM requests `get_collections` makes: one each for standard and smart, and one for
each of the two virtual types the plugin syncs.
