...
Collections, Smart collections and Autogenerated are RomM's own headings; Favorites, Franchises and IGDB collections are
this page's.
...
2. **one sentence, worded for that kind, on what turning one of its collections on does**: it adds all of that
   collection's games to Steam, **including games on platforms the reader does not sync**, and groups them as
   `RomM: [name] (host)`. The narrow tab says neither;
...
standard and smart collections, which is what the Owner column shows. `get_collections` receives both and forwards
neither today, so both are the build's to carry through, and the page still costs the four RomM requests
`get_collections` makes: one each for standard and smart, and one for each of the two virtual types the plugin syncs.
...
collections off no write reaches a collection of another user.
...
Steam Library setting that turns that mode on, and the user guide. New suffixes rename the Steam collections a
`by_label` library already has; that is accepted, and the old-named collections must not be left behind. The keys on the
wire and in `settings.json` stay `standard` / `smart` / `virtual`.
