...
turns that mode on, and the user guide. Under `by_label` a standard collection carries no suffix, and every other kind
carries a short singular form of its name: `(Smart)`, `(Franchise)` and `(IGDB Collection)`, with `(Autogenerated)` for
a virtual collection of no known type. `(IGDB Collection)` keeps its capital because Steam tells collections apart
without regard to case, and Tender finds an existing one the same way, so a change of case alone would not rename a
collection that already exists. Today the suffixes read `(Standard)`, `(Smart)`, `(Franchise)`, `(IGDB Collection)` and
`(Virtual)`. [steam-non-steam-shortcuts.md](steam-non-steam-shortcuts.md) § Collection naming mode lists them and
requires them to match the frontend's kind words exactly; singular suffixes cannot meet that requirement, so the build
replaces it there with the rule stated here. Because a standard collection carries no suffix, a standard collection
