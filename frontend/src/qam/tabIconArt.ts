...
export const TAB_ICON_ARC = {
  ...
  width: 15.5,
  head: "176.93,91.51 180.75,65.39 155.38,76.26",
  headRound: 4.3,
} as const;
...
/** One button position, punched out of the bar it sits on. */
export interface TabIconHole {
  readonly cx: number;
  readonly cy: number;
}

/** All four of them, in the mark's own draw order. */
export const TAB_ICON_HOLES: readonly TabIconHole[] = [
  { cx: 100.9, cy: 73.2 },
  { cx: 66.0, cy: 100.8 },
  { cx: 99.1, cy: 126.8 },
  { cx: 134.0, cy: 99.2 },
...
/** The radius each one is cut at, narrower than the mark's own dot. */
export const TAB_ICON_HOLE_R = 12.0;
