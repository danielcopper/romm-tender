...
export const STEAM_LOOKUPS: readonly SteamLookup[] = [
  ...
  // | `quickAccessMenuClasses` | `TabGroupPanel` at module scope (`utils/qamExpansion.ts:43-45`), read into the selectors of the injected sheet (`:99-100`); `ActiveTab` in `useWideQamPanel`'s effect (`:189`) | `TAB_PANEL_SELECTOR` becomes `PANEL_ID_SELECTOR`, the panel's id; `owningTabActive` defaults to true (`:196`, the default argued just above it) and the `MutationObserver` guarded at `:205` is never constructed | the sheet matches the panel by id instead of by class, and the expansion is taken whether or not the page's own tab is the active one and is not re-synced on a tab switch — a leaked expansion the QAM-close, unmount and dismount paths still clear |
  //
