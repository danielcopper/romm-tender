 ...
 * and reads the same numbers (`frontend/src/utils/syncRunView.ts` states the
 * rule). What it does key on the run's end is its OWN reads — the run list, the
 * stats and the session-budget reading all describe the run that just stopped —
 * and those are observations of the frame, not a second announcement of it.
 * They are taken on a stop carrying a terminal stage, because this page also
 * stops a run of its own making: the optimistic frame it retracts when a
 * preview answers.
 *
