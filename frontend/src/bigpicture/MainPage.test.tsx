...
//      ever runs. Its unsubscribe is pinned in
//      frontend/src/utils/migrationStore.test.ts.)
...
vi.mock("@decky/ui", async () => {
  ...
  return {
    ...
    // Mirrors the shared stub in frontend/src/test-setup.ts: the description
    // rides in a SIBLING span, never inside the button, so it stays assertable
    // (a dropped prop would make every assertion of its ABSENCE vacuously true)
    // without leaking into the button's own textContent, which is how tests
    // here identify buttons by label.
    ButtonItem: ({
      children,
      onClick,
      disabled,
...
describe("MainPage", () => {
  ...
  // pinned where it lives (frontend/src/utils/syncRunView.test.ts). What is
  // measured here is the subscription Main installs: a mounted instance keeps
  // re-rendering, and what it feeds the estimator with.
  ...
  describe("pending preview restored on mount", () => {
    ...
    // preview (frontend/src/bigpicture/SyncPage.test.tsx). What Main reads the
    // store for is its SLOT: a preview still worth reviewing is worth a row
    // that says so.
