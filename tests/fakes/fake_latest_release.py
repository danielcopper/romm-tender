...
class FakeLatestRelease:
    ...
    class of failures the adapter swallows. ``calls`` is what a throttle test
    asserts on. Set ``raises`` to make the seam misbehave the way its Protocol
    says it never will — the service's silence has to hold against that too.
    """
