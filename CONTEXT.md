...
(the `enabled_collections` bucket keys, `WorkUnit.collection_kind`, and `CollectionSyncState.collection_kind`), which is
RomM's API vocabulary. **Standard** is a manually-created collection (RomM's ownership-carrying kind — its ROM
membership is hand-picked; the auto-managed favorites collection is a Standard one); **Smart** is a saved-search whose
membership resolves at query time; **Virtual** is an ownerless grouping RomM derives from IGDB metadata, of one of two
`virtual_type`s — `franchise` or `collection` (an IGDB collection). Only Standard and Smart are stampable for the
incremental skip. Internal-name history: the first kind was called `user` until #1539 renamed it `standard` (display
"My" → "Standard").

**The names a reader sees are not the literals.** The QAM's Collections tab labels the kinds **Standard**, **Smart** and
**Virtual**, with **Franchise** / **IGDB Collection** inside Virtual. The names decided under #1833, and not yet built,
are RomM's own headings: **Favorites** for the favorites collection, **Collections** for the rest of `standard`, **Smart
collections**, and **Autogenerated** — RomM's UI word for `virtual` — over **Franchises** and **IGDB collections**. The
literals do not follow either set of names.
...
setting valued `all` (every collection the server lists, including other users' public ones) or `own` (only the
signed-in user's own collections). **A sync scope, not a filter**: under `own`, a foreign standard or smart collection
is hidden from the list **and** left out of the sync, even one switched on earlier — once the signed-in user's id is
known, since until then no collection counts as foreign. Orthogonal to the collection kind axis: it decides by owner
(`is_own` / `user_id` vs `romm_user_id`), not by kind, and virtual collections have no owner so it never touches them.
The stored value stays `own` / `all` (the value was left `own` to avoid a settings migration, #1539). The QAM presents
it as **Mine** / **All**; decided under #1833, and not yet built, is a single toggle, **Other users' collections** — on
is `all`, off is `own`. _Avoid_ calling it a filter: the control that looked like one is why the toggle replaces it.
...
`RomM: [<name> (Smart)]`, `RomM: [<name> (Standard)]`, and `(Virtual)` for a virtual collection of no known type. The
label is the fine label, not the coarse kind — franchise and IGDB-collection are both `kind="virtual"`, distinguished by
`virtual_type` (see the **Collection kind** entry above). Computed backend-side at the reporter's union key
...
The suffixes decided under #1833, and not yet built, follow the kind names a reader sees: a standard collection carries
**no** suffix (`RomM: [<name>] (<host>)`), and the others carry `(Smart)`, `(Franchise)` and `(IGDB collection)`, with
`(Autogenerated)` for a virtual collection of no known type. That renames Steam collections a `by_label` library already
has, which is accepted, and the old-named ones must not be left behind.
