...
- **Every RomM request goes out through `RommHttpAdapter._urlopen`, the single point that clears the known-unreachable
  state** — check — `scripts/check_urlopen_choke_point.py`; which requests may skip the retry ladder, and which pass
  `romm_origin=False` because they do not talk to RomM, stays prompt-only in `.claude/rules/romm-http.md`
...
- **Where this program's directories are is resolved once from the environment, and every consumer reads them off
  ...
  `RuntimeBundle` carries no directory. Nothing tells the seven fields apart: re-derivable data goes under `cache_dir`
  and data that is not under `data_dir`, and nothing under `bin_dir` is ours to remove
- **The identifier's three homes are never derived from one another — in particular `APP_DIR_NAME`
  ...
  `tests/domain/test_identity.py::TestTheIdentifierStaysInTwoPlaces`; the reverse fold through an aliased import and the
  third home, the frontend's `SESSION_BREADCRUMB_KEY`, are prompt-only. The three homes are listed in
...
- **A library-sync seam is held only by the module owning the job it belongs to: `active_core` / `disc_resolver` by
  ...
  `services/library/session_budget.py`, and `artwork` by `services/library/cover_preparer.py` **and**
  `services/library/reporter.py` — the one confinement with two owners, never a third. The `service.py` façade may
  **pass** a seam on (a call's keyword-argument value, a `LibraryServiceConfig` field) and may not **use** one** — check
  — `scripts/check_seam_owner.py`; prompt-only: `shortcut_launch_resolver`'s install-path read UoW is never held across
  the `active_core` seam's calls (`check_uow_seam_nesting.py` catches only the inline form)
- **A module declared read-only calls no repository write — `services/library/local_library_reader.py` to start** —
  check — `scripts/check_read_only_module.py`, with `tests/scripts/test_check_read_only_module.py` pinning its
  name-shape read/write split for every repository method; "repository writes are named as writes" is prompt-only: a
  write named like a read (`get_or_create`) passes the gate
...
- **The KIND of run a `sync_progress` frame belongs to is stated on it (`runKind`), never inferred from it — and a frame
  that states none is rendered as neither of the two answers** — test + prompt-only —
  `tests/services/library/test_sync_orchestrator.py::TestRunKindOnTheWire`,
  `tests/services/library/test_state.py::TestRunKind`, `frontend/src/utils/syncRunView.test.ts` and
  `frontend/src/bigpicture/MainPage.test.tsx`; the join is prompt-only: every frame builder carries the key, every start
  path stamps the kind it is, and no reader spends the absent case on either answer
...
- **A firmware answer nothing could establish is `unknown`, never `not_needed` — and the distinction survives every
  ...
  `reading_complete_for` refuses `None`, and `declaration="packaged"` with an empty requirement list counts as unread
- **A firmware row the RomM library does not hold (`on_server: False`) counts towards readiness, and never towards a
  ...
  `frontend/src/bigpicture/library/PlatformsTab.test.tsx`; prompt-only: the three axes —
  `domain/bios_status.py::count_required`, `services/firmware/status.py::_bios_aggregates` and `isFetchable`
  (`frontend/src/utils/biosFetchable.ts`) — stay apart, and the platform detail and the game page's BIOS tab call the
  one `isFetchable` rather than copying it, while the game page's row-visibility rule stays its own
...
- **The plugin attaches a configured custom header to a RomM-origin request and to no other request it issues, and never
  ...
  `_apply_origin_headers` and `download_external` never does, and the attachment order is unpinned. `Host` and
  `Content-Length` are refused only by `RESERVED_NAMES`. A cross-host redirect still carries the header and the bearer
  to the foreign host
- **`dist/globals.js` is never evaluated into Steam where Decky Loader is serving, and what decides that is read from
  ...
  `tests/host/inject/test_injector.py::TestBesideDeckyLoader`; prompt-only: the file list comes only from
  `host/inject/bundles.py` over `host/inject/machine.py`'s answer, never from another caller or a window probe
- **Where the globals bundle is loaded, its installer is CALLED between the import that defines it and the panel import,
  ...
  `tests/host/inject/test_bootstrap.py::TestItRunsUnderNode`, `test_bundles.py` and `TestTheInstallersName`;
  prompt-only: the stub report's keys (`installed`, `steamReady`) agree with `GlobalsReport` in
...
- **An injection that could not be observed is never counted as a crash, and this process answers for its own record
  ...
  a record without counting it whenever nothing was established
- **Tender's Quick Access entry composes with Decky's rather than going through it, and everything it binds to the Quick
  ...
  `frontend/src/qam/quickAccessEntry.test.ts`; that the renderers are found and patched is device-only. Prompt-only: the
  entry's marker and key are `tender`, never `decky`; nothing writes `window.__TABS_HOOK_INSTANCE` or calls its `add()`,
  and neither becomes a presence check; no tab array is held anywhere; the placement is re-asserted on every render
  pass; nothing binds to the Quick Access window at module scope
- **Tender's section reaches Steam's game page through the ROUTE component's `renderFunc`, and never through the page
  ...
  function, and the start-up check's `AppDetailsRoute` and `appDetailsClasses` cost `feature`, never `panel`
...
- **No UoW-opening seam (ActiveCoreResolver, RelaunchOptionsResolver, uow_factory) is called while a UoW is open on the
  same path** — check — `scripts/check_uow_seam_nesting.py`, first seam family
- **No file-I/O seam is called while a UoW is open — a Unit of Work wraps database reads and writes, never file or
  server I/O** — check — `scripts/check_uow_seam_nesting.py`, second seam family (`IO_SEAM_METHODS`); prompt-only: a
  seam that list does not name — `SteamConfigStore.grid_dir()` and `check_retroarch_input_driver()` today — and a seam
  passed as a bound method, which is how `read_shortcut_exes` is reached
...
- **No module in `services/`, `bootstrap/`, `adapters/`, `domain/`, `lib/` or `models/` crosses the ~1000-LOC
  decomposition threshold, and the ones already over it may not grow** — check — `scripts/check_module_size.py`; a
  grandfathered ceiling goes up only for a change that adds no code, with the reason at its `ALLOWLIST` entry
...
- **Frontend direction: `frontend/src/utils/` and `frontend/src/api/` never import either surface
  ...
  (`import-x/no-restricted-paths`, `import-x/no-cycle`), kept live by `frontend/src/eslintBoundaries.test.ts`
...
- **A transport failure and a callable's own failure never arrive in the same shape, on either end** — test +
  ...
  `.claude/rules/host.md`, the vocabulary `backend/host/protocol.py`. Prompt-only: `connection_lost` is spelled once in
  Python and once in TypeScript, and nothing holds the two equal
...
- **Every value the panel imports from `@decky/ui` is classified by the start-up check** — test —
  `frontend/src/boot/steamModules.test.ts`, which derives `ASKED_LIVE` through
  `frontend/src/test-utils/jsFunctionScanner.ts`; every entry reads the module registry or a bootstrap global, never
  what Steam has mounted or focused, and whether any other classification is true is unchecked
- **Whether every search answered and whether the panel may MOUNT are two questions, and a miss that costs less than the
  ...
  required), `frontend/src/index.test.tsx` and `frontend/src/boot/steamModules.test.ts`; prompt-only: the join across
  `checkSteamModules`, `index.tsx` and `describeSurvivedMiss`, the notice from `notificationsMissing` over
  `NOTIFICATION_LOOKUPS`, and that a name leaves `panel` only once its every consumer is read
- **The start-up failure page names the copy of `@decky/ui` that actually ran the search that missed, and the repair
  that follows from it** — check + test + prompt-only — the stamp by `frontend/scripts/check-bundle-shape.mjs`, every
  `SEARCH_OWNERS` verdict's sentence by `frontend/src/boot/steamModules.test.ts` and `StartupFailurePanel.test.tsx`;
  prompt-only: the join across `rollup.config.js`, `boot/searchingCopy.ts`, `boot/steamModules.ts` and `index.tsx`,
  which resolves it once; a miss confined to names `@decky/ui` does not export names no copy and no repair, which leaves
  `ControllerGlyph`'s repair unnamed; the copy is read off the stamp, never a runtime probe; `in DFL` is never asked
  about a name `@decky/ui` never exported; reading `DFL` never throws, and an unreadable one is nothing established, not
  an absence
- **A coverage exclusion names a property of the code, never a place: every frontend-scoped entry stands in BOTH
  ...
  is listed** — check — `scripts/check_coverage_exclusions.py`; prompt-only: a module filed into an admitted folder
  entry (`frontend/src/types/`, `frontend/src/test-utils/`), and the truth of a marker's sentence
...
- **Every tree under `backend/_vendor/` is pinned by the `<pkg>.SHA256SUMS` beside it: every manifest entry under
  ...
  prompt-only: the manifest itself, regenerated only as the last step of a deliberate re-copy and never to answer a
...
- **The release tarball is what the installer expects — one top-level `romm-tender/`, the files an install starts from
  ...
  `sha256sum -c` accepts** — check — `scripts/check_release_tarball.py`, in CI's build job and in the release job
...
- **A firmware row's presence comes from the resolver wherever the resolver declared it; the plugin's own filesystem
  ...
  single crossing point, and nothing enforces it; `present is None` reads as absent. A withheld verdict is not an
  absence: its cause is read off the row's caveat codes and a declared file's `checked`, never off the verdict, and
  nothing checks that a consumer keeps `checked`'s values apart
...
- **The console's own firmware demand is a value of its own (`system_image`) and is never folded into a count, and the
  ...
  `tests/domain/test_bios_status.py::TestClassifySystemImage` and `::TestTheVerdictOverTheSystemImage`,
  `tests/services/test_firmware.py::TestTheConsolesOwnFirmwareDemand`, `frontend/src/bigpicture/BiosTab.test.tsx`,
  `frontend/src/bigpicture/library/PlatformsTab.test.tsx`, and the wording locks `biosSummary.test.ts` and
  `biosHeldRatio.test.ts` over `frontend/src/test-utils/componentSources.ts`. Prompt-only: the demand comes from the
  table and the presence from our rows, never from `requirements_met`; every surface words these states through
  `frontend/src/utils/biosSummary.ts`, which alone holds their order (`"absent"` before the level's decline; with both
  ignorances it names the withheld row); `PlatformDetail`'s `nothingEstablished` decides wording alone; the play row's
  badge (`playSection.ts::extractBiosInfo`) rises on `"absent"` and never on `"unsettled"`; a new wording, or one in a
  `.ts` helper such as `panelState.ts`, escapes the locks. Per core, `required` and `needs_one_of` are two speakers,
  never rewritten into or folded onto each other; a core gets `needs_one_of` only where it marks nothing required, and
  `system_image_candidate` stays a strict subset of the rows `classify_system_image` weighs
- **Which emulator a set of answers is about is ONE pick per scope — a platform's, and a ROM's — and every answer in
  ...
  `tests/services/test_firmware.py::TestOnePlatformOneEmulator`,
  `::TestDownloadRequiredFirmware::test_it_fetches_what_the_platforms_own_pick_calls_required`,
  `TestTheAnswerNamesTheEmulatorItJudgedBy` and `::TestOneRomOneEmulator`; prompt-only: every platform site resolves
  through `domain/emulator_commands.py::resolve_platform_option`, `.label` and `.emulator` come off one call, the key is
  the emulator identity and never `.core_so`, and both game-page services ask
  `ActiveCoreReader.active_emulator_for_rom`, never `active_core_for_rom`. Tests on the bare `FakeCoreInfoProvider`
  exercise a weaker default than the live adapter
- **A platform's BIOS answer is asked for one platform at a time, and a row that has not got one yet is never rendered
  ...
  state), and the hook's `alive` guard is unpinned
...
- **Every destructive op has backup-or-confirm; never delete data that exists nowhere else** — prompt-only — save-file
  ...
  Two deletes take the confirm leg: the removed-game cleanup's unselected installed ROM content, and the adopt dialog's
  **replace** exit for the ROM; an adoption's Overwrite of save and savestate files takes the backup leg through the
  same funnel
- **A BIOS file is deleted only where a `downloaded_bios` record names it under one of the platform's firmware slugs,
  ...
  still on disk and alone gates the Delete offer; the three buttons share `PlatformBiosDeleter._delete_recorded_io`,
  never a copy of it
...
- **Which files a game's save consists of is the EMULATOR's answer, read live, and four of its five states refuse the
  ...
  `tests/domain/test_save_answer.py` and `tests/services/saves/test_save_shape_gate.py`; prompt-only: every per-ROM
  entry point refuses through `sync_engine/_shape_refusal.py` and every sync path crosses the
  `MatrixExecutor.sync_rom_saves` backstop; configuration-role files are held back through `SaveAnswer.synced_files` by
  the denial `CONFIGURATION_ROLES`, never an allow-list, and nothing reads `components` directly; a rendering reads
  `unestablished`'s shape and `content_installed` beside the state; the question carries the ROM's real content path;
...
- **`applied_launch_options` is written only by the six recorded-state writer sites (sync ack-commit, download-complete,
  ...
  `set_applied_launch_options` / `record_applied_launch_options` call-site audit
...
- **Every path on which the user answers the preview question leaves a live snapshot on neither side — the
  ...
  server-side; nothing mechanical can tell
...
- **Every installed-content mutation is authorized by a descriptor-relative no-follow claim (root identity, descendant
  ...
  **bundle**, not the caller: a source a sealed bundle holds consumes the bundle's digest-bound claim, a source it did
  ...
  and only where a surviving install row proves the path. Both keep staging rename, mount checks, no-follow traversal
  and exact-identity revalidation under writer exclusion across each unlink; a content-bound removal leases the whole
  tree up front, an identity-only directory per unlink, so a writer mid-loop yields a reported partial removal** —
...
- **Every write into per-rom detail state that crosses an `await` is bound to a rom identity — the store
  ...
  panel's state, event and tab-content modules via `RomBinding` (`bindRom`, `bindRomInState`); `AchievementsTab.tsx` by
  its rom-id key. Two answers for the SAME rom are ordered by a sequence taken at issue (`loadSeq`, `takeReadTicket`).
  Ordered but unbound: the two identity writes and the panel's lazy lane; neither bound nor ordered: the store's
...
- **Every row a reader must be able to reach on a QAM page is a row Steam can focus — a toggle, a button, or a
  ...
  `tender/qam-focusable-row` for the syntactic slice; prompt-only: focus order, runtime reachability, edge revelation
  (`ScrollRegion`'s `revealEdge`, whose decision `frontend/src/bigpicture/layout/ScrollRegion.test.tsx` pins), scrolling
  geometry and controller behaviour, and that a page's controls sit above its long focusable lists, not below
- **A list-and-detail page opens on the row it was opened WITH, not on its first row** — test + prompt-only —
  `ListDetail.test.tsx` and `WidePage.test.tsx`, each over a non-first row; prompt-only: the join across
  `utils/entryFocus.ts` (`ENTRY_STOP_ATTR`, `pageEntryStop`), `bigpicture/layout/WidePage.tsx`,
  `bigpicture/layout/ListDetail.tsx` and every page passing a starting selection, and that the mark sits on the
  `display: contents` wrapper around each row, never on the row
